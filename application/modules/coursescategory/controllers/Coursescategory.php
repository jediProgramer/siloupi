...
class Coursescategory extends CI_Controller {
	private $namespace = 'coursescategory';
	private $model_name = 'coursescategory_model'; 
	...
	{
		$data = $this->initView("Entri Data Kategori Mata Kuliah");
	...
	{
		$data = $this->initView("Impor Data Kategori Mata Kuliah");
		$data['datauser'] = $this->model->getUserData($data['idusers']);
	...
	function delete()
    {
		...
		$this->model->delete('idcoursescategory',$id,'idcurriculum',$idc);
	...
	function export()
	{
        ...
        $sheet->setCellValue('A1', 'idcoursescategory'); 
        $sheet->setCellValue('B1', 'idcurriculum');
        $sheet->setCellValue('C1', 'coursescategory');

		$i = 2;
		foreach ($data as $d) {
			$sheet->setCellValue('A'.$i, $d['idcoursescategory']); 
			$sheet->setCellValue('B'.$i, $d['idcurriculum']);
			$sheet->setCellValue('C'.$i, $d['coursescategory']);
			$i++;
		}
	...
	function import(){
		...
		if(isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
			...
			if($sheetData[0][0] == 'idcoursescategory'){
