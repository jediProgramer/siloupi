...
$('#addGraduationPeriod').validate({
	...
	messages: {
		graduate_name: {
		required: "<?php echo lang('graduate_name_error_msg');?>"
	  },
	},
	submitHandler: function (form) {
		var years = $('#years').val();
		var graduate_name = $('#graduate_name').val();
		var month_graduationperiod = $('#month_graduationperiod').val();
		$.ajax({
			type : "POST",
			url  : "<?php echo site_url('param/savegraduationperiod')?>",
			dataType : "JSON",
			beforeSend :function () {
					swal({
			...
			data : {years:years, graduate_name:graduate_name, month_graduationperiod:month_graduationperiod},
			success: function(value){
				if (value.msg == 'true') {
					swal({
					  type: "success",
					  title: "<?php echo lang('param_graduationperiod_add');?>",
					  text: value.msg_success,
					  confirmButtonColor: '#1e3d73',
					});
					$('.swal2-confirm').click(function(){
						window.location.href = "<?php echo base_url() ?>param/graduationperiod";
					});
				}
				else
				{
...
$('#editGraduationPeriod').validate({
	...
	submitHandler: function (form) {
		...
		var graduate_name = $('#graduate_name').val();
		var month_graduationperiod = $('#month_graduationperiod').val();
		var idgraduation = $('#idgraduation').val();
		$.ajax({
			type : "POST",
			url  : "<?php echo site_url('param/updategraduationperiod')?>",
			dataType : "JSON",
			beforeSend :function () {
					swal({
						title: "<?php echo lang('waiting');?>",
			...
			data : {years:years, graduate_name:graduate_name, month_graduationperiod:month_graduationperiod, idgraduation:idgraduation},
			success: function(value){
				if (value.msg == 'true') {
					swal({
					  type: "success",
					  title: "<?php echo lang('param_graduationperiod_edit');?>",
					  text: value.msg_success,
					  confirmButtonColor: '#1e3d73',
					});
					$('.swal2-confirm').click(function(){
...
$('.deleteGraduationPeriod').on('click',function(){
	$.ajax({
		type : "POST",
		url  : "<?php echo site_url('param/deletegraduationperiod')?>",
		dataType : "JSON",
		beforeSend :function () {
				swal({
					title: "<?php echo lang('waiting');?>",
		...
		data : {idgraduation:$(this).attr("data-id")},
		success: function(value){
			if (value.msg == 'true') {				
				swal({
				  type: "success",
				  title: "<?php echo lang('param_graduationperiod_delete');?>",
				  text: value.msg_success,
				  confirmButtonColor: '#1e3d73',
				});
				$('.swal2-confirm').click(function(){
